...
def download_report():
    ...
    cursor = mysql.connection.cursor()
    cursor.execute(
        'SELECT * FROM files WHERE idTeachers = %s', (code,))
    ...
    cur.execute(
        'SELECT * FROM users WHERE codeUDG = %s', (code,))
    res = cur.fetchone()
    ...
    if res:
        ...
        pdf.set_text_color(255)
        pdf.cell(w=40, h=15, txt='Ciclo', border=1, align='C', fill=True)
        pdf.set_text_color(0)
        pdf.multi_cell(w=120, h=15, txt=str(
            # result.row['ciclo']) """
            "--"), border=1, align='C', fill=0)
...
def getTeacher():
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        cursor.execute(
            'SELECT files.date, files.cycle, files.id, files.idTeachers, files.courseName, files.evidenceType, files.shift, files.fileName FROM files WHERE idTeachers = %s', (code,))
        res = cursor.fetchall()
...
def plot():
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        cursor.execute(
            'SELECT courseName, COUNT(*) AS qty FROM files WHERE idTeachers = %s GROUP BY courseName', (code,))
        res = cursor.fetchall()
...
def fileView(id):
    ...
    if 'loggedin' in session:
        ...
        cursor = mysql.connection.cursor()
        cursor.execute(
            'SELECT files.file, files.mimetype FROM files WHERE id = %s', (id,))
...
def view(codigo):
    ...
    if 'loggedin' in session:
        cursor = mysql.connection.cursor()
        cursor.execute(
            'SELECT files.id, files.academyName, files.courseName, files.evidenceType, files.shift, files.fileName FROM files WHERE idTeachers = %s', (codigo,))
        res = cursor.fetchall()
